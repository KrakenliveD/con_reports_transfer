...
# 並按真實數據補錄吉圖諮詢/尼爾森/飛瓜數據/聯合資信/源達信息等非券商。
NON_BROKER_ORGS = [
    "头豹研究院", "硕远咨询", "蔚云出海", "世界银行",
    "携程集团", "淘天", "阿里巴巴", "亿欧智库",
    "创业邦", "科睿唯安", "怡安企业服务",
    ...
    "灼识投资咨询", "艾瑞",
    # 2026-08-17 實測漏網補錄
    "吉图企业管理咨询", "尼尔森", "飞瓜数据", "联合资信评估", "源达信息",
    "中证鹏元", "中国信通院", "腾讯研究院", "甲子光年智库",
    "中国电力企业联合会", "中国汽车流通协会", "世界黄金协会",
    "国家能源局", "国际能源署", "清华大学", "北京航空航天大学",
    "首都医科大学附属", "欧洲疾病预防控制中心", "美国农业部",
    "普罗维植", "一带一路", "语言教育文化组织联盟",
    "上海市住房和城乡建设管理委员会",
    "北京智信联成管理咨询", "北京顺为人和企业咨询", "山东涌益信息咨询",
...
# NON_BROKER_ORGS 保留為例外覆蓋（防「证券时报」「XX证券服务」等含证券字樣的非券商）。
BROKER_KEYWORDS = ["证券"]
...
def is_broker(org_name: str) -> bool:
    ...
    if any(k in org_name for k in BROKER_KEYWORDS):
        return True
    return any(a in org_name for a in BROKER_ABBREV)
